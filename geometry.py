...
class Point:
	...
	__repr__ = lambda self: '(' + format(self.x,'.3f') + ',' + format( self.y,'.3f') + ')'
...
class Angle:
	...
	__neg__ = lambda self: Angle(Point(self.cos, -self.sin))
...
class Segment:
	...
	__str__ = lambda self: "Segment from %s to %s" % (str(self.start), str(self.end))
